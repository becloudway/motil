...
test('testString is required and should be valid', () => {
    let validationEngine = new ValidationEngine(generateTestObject(), {testString: "required"});
    let result: ValidationResult = validationEngine.processRules();
...
test('testString is required, should match the regex and be atleast 3 characters long', () => {
    let validationEngine = new ValidationEngine(generateTestObject(), {testString: "required|min:3|isRegex:^some_[a-z]+$"});
    let result: ValidationResult = validationEngine.processRules();
...
test('testString is required, should match the regex and be max 3 characters long - result should be false', () => {
    let validationEngine = new ValidationEngine(generateTestObject(), {testString: "required|max:3|isRegex:^some_[a-z]+$"});
    let result: ValidationResult = validationEngine.processRules();
